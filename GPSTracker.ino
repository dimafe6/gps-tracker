...
void onWiFiConnected() {
    ...
    synchronizeConfigurationFromServer();
    runGPSTimer();
}
...
void synchronizeConfigurationFromServer() {
  Serial.println("Synchronize configuration from server...");
  
  String settingsJson = API.get("/settings").responseText;
...
void saveConfiguration(const char *filename, const Config &config) {
  ...
  if(SD.remove(filename)) {
    ...
    if (root.printTo(file) == 0) {
      ...
      Serial.print("Sending configuration to server...");
      Serial.println(API.post("/settings", settings).code == 200 ? "OK" : "Error");
    }
